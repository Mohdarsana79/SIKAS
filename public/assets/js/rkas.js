document.addEventListener('DOMContentLoaded', function() {
    ...
    updateTahapCards();
    initializeSalinDataButton(); // Inisialisasi tombol salin data
    ...
    function initializeRkasSearch() {
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            // JANGAN show loading immediately - tunggu sampai debounce
            // showRkasSearchLoading(true); // HAPUS BARIS INI
            
            if (searchTerm.length === 0) {
                if (isSearching) {
                    showRkasSearchLoading(false);
                    resetRkasSearch();
                    isSearching = false;
                }
                return;
    ...
    function showRkasSearchLoading(show) {
        const searchInput = document.getElementById('SearchInput');
    ...
    function showSkeletonLoading(month, show) {
        const tableBody = document.getElementById(`table-body-${month.toLowerCase()}`);
        
        if (!tableBody) return;
        
        if (show) {
            // Simpan konten asli sebelum menampilkan skeleton
            if (!tableBody.hasAttribute('data-original-content')) {
                tableBody.setAttribute('data-original-content', tableBody.innerHTML);
            }
    ...
    function performRkasSearch(searchTerm) {
        ...
        if (!tahun) {
            ...
            showRkasSearchLoading(false);
            showRkasSearchError('Tahun anggaran tidak ditemukan');
            return;
        }
        ...
        .then(data => {
            showRkasSearchLoading(false); // Pastikan loading di input dimatikan
            if (data.success) {
        ...
        .catch(error => {
            ...
            showRkasSearchLoading(false); // Pastikan loading dimatikan saat error
            showTableLoading(currentMonth, false);
            resetRkasSearch();
            showRkasSearchError('Terjadi kesalahan saat mencari data: ' + error.message);
        });
    }

    function handleRkasSearchResponse(data, searchTerm, currentMonth) {
        const tableBody = document.getElementById(`table-body-${currentMonth.toLowerCase()}`);
        
        if (!tableBody) {
            showRkasSearchError(`Tabel untuk bulan ${currentMonth} tidak ditemukan`);
            return;
        }
            ...
            tableBody.innerHTML = `
                <tr>
                    <td colspan="11" class="text-center py-5 text-muted">
                        ...
                        Tidak ditemukan data RKAS dengan kata kunci: "<strong>${escapeHtml(searchTerm)}</strong>" di bulan ${currentMonth}
                    </td>
                </tr>
            `;
        } else {
        ...
        showRkasSearchInfo(data.data.length, searchTerm, currentMonth);
    ...
    function showRkasSearchInfo(count, searchTerm, month) {
    ...
    function showRkasSearchError(message) {
        Swal.fire({
            icon: 'error',
    ...
    function removeRkasSearchInfo() {
        // Tidak perlu implementasi khusus untuk sekarang
    }

    function resetRkasSearch() {
        const activeTab = document.querySelector('#monthTabs .nav-link.active');
    ...
    function reattachRkasEventListeners() {
    ...
    /**
     * Tampilkan SweetAlert konfirmasi salin data
     */
    function showSalinDataConfirmation() {
        const tahun = document.querySelector('input[name="tahun_anggaran"]')?.value;
    ...
    /**
     * Proses salin data - return Promise untuk SweetAlert
     */
    function processSalinData() {
        const tahun = document.querySelector('input[name="tahun_anggaran"]')?.value;
    ...
    // PERBAIKAN: Pastikan tab aktif tersimpan dengan benar
    document.querySelectorAll('#monthTabs .nav-link').forEach(tab => {
        tab.addEventListener('click', function() {
            const month = this.getAttribute('data-month');
            localStorage.setItem('activeRkasTab', month);
            console.log('🔧 [TAB DEBUG] Saved active tab to localStorage:', month);
        });
    });
